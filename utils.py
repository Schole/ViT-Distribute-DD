...
class StyleTranslatorSharedDataset(Dataset):
    def __init__(self, images, styles):
        self.n_c, self.ipc, h, w = 10, 50, 32, 32
        self.styles = styles
        self.images = images.detach().float()
...
def epoch(mode, dataloader, net, optimizer, criterion, args, aug, trans=None):
    ...
    for i_batch, datum in enumerate(dataloader):
        ...
        if lab is None:
            print(f"Skipping batch number {i_batch} due to None label")
            continue
...
class StyleTranslator(nn.Module):
    ...
    def __init__(self, in_channel=3, mid_channel=3, out_channel=3, image_size=32, kernel_size=3):
        super().__init__()
